...
- **三个段位** —— 高阶 / 特级 / 究极大师，可单独查看，或按 1 : 2 : 3 段位深度权重综合（究极大师权重最高，详见[方法论](docs/METHOD.md#3-rank-tiers-and-skill-depth-weights)）。
...
**4. 段位综合 → COMB。** 将各段位均值按段位深度权重
`W = {高阶: 1, 特级: 2, 究极: 3}` 在有数据的段位上综合：
...
相性值。这是一个刻意偏向「偏差而非方差」的取舍 —— 究极大师人群最小、噪声最大，却
