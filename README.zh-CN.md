...
**4. 段位综合 → COMB。** 将各段位均值按人口权重
`W = {高阶: 3, 特级: 2, 究极: 1}` 在有数据的段位上综合：

```text
COMB(O) = Σ_r  W_r · m̄(O, r)  /  Σ_r  W_r
```

高阶大师权重最高（人群最大、噪声最小），究极大师最低。单段位视图
（`--profile` 选定某段位）直接使用 `m̄(O, r)`。另外单独报告每个对手的
