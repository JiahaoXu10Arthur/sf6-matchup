...
| **教练** | 排序的"接下来该练什么" —— 你的对位按 频率 × 低于技能匹配基准的幅度 × 置信度 评分，标注个人短板/普遍困难，并附推荐副角 |
...
- **使用率 × 胜率元数据图** —— 全角色的热度对胜率散点图（高估/低估象限）。
- **教练** —— 把你导入的对位按"接下来该练什么"排序：频率 × 低于技能匹配基准的幅度（你的战绩向接近你 MR 的对手加权）× 置信度，标注个人短板/普遍困难，并附推荐副角。
...
# 应用：http://localhost:8741/web/   （相性表 · 柱状图 · 副角推荐 · 使用率 × 胜率 · 教练 · 个人侦察 · 趋势）
```
...
在「个人侦察」标签页导入战斗记录后，**个人模式** 开关会让副角推荐与
相性表/柱状图按你自己的天梯数据呈现。每个对位都是你的战绩向全球基准收缩的结果
（Beta-Binomial，约 20 局先验），因此在你没有对局数据的地方会精确回退到全球数据 ——
关闭开关时各视图与全球默认完全一致。副角推荐按你实际失利的对位排序；相性表与柱状图
标注你的胜负与超常/不及预期的差值；个人侦察表则为每个劣势对位给出推荐副角；而
**教练** 视图会把每个对位按"接下来该练什么"排序。个人数据全程留在浏览器中。

个人侦察还会维护一个**名册**（roster）—— 你自己和其他玩家 —— 以 CFN id 为键，名称可编辑。
每个档案会在多次导入之间**累积**对战记录（按 replay id 合并去重，并通过浏览器 IndexedDB 持久化），
因此即便单次 Buckler 抓取只能取到最近约 100 场排位，数据深度也会随时间增长。按月/按补丁加权的
视图、每个对位的补丁前后变化（Δ），以及**趋势**图都会读取当前档案；你还可以把整个名册
**导出/导入**为一份归你所有的 JSON 文件。所有数据都不会上传。
